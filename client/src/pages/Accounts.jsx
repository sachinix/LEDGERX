...
function Accounts() {
  ...
  const copyAccountId = async (value) => {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[2rem] border border-slate-800 bg-slate-900/95 p-8 shadow-soft">
        ...
        <p className="mt-2 text-sm text-slate-400">Review your active accounts, balances, and currency details.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {accounts.length === 0 && (
          <div className="rounded-3xl border border-slate-800 bg-slate-900 p-8 text-center shadow-soft">
            <p className="text-sm text-slate-400">No accounts found. Create an account from the backend or contact support.</p>
          </div>
        )}
      </div>
    </div>
